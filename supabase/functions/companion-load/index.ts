...
Deno.serve(async (req: Request) => {
  ...
  const { data, error } = await sb.from("aria_saves").select("save, updated_at").eq("client_id", clientId).maybeSingle();
  if (error) return out({ error: "db", detail: error.message }, 500);
  return out({ save: data ? data.save : null, updatedAt: data ? data.updated_at : null });
});
